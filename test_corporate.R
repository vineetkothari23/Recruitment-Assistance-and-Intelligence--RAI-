...
body <- dashboardBody(
  tabItems(
    ...
    tabItem("Analysis",
            fluidRow(
              valueBoxOutput("oi",width = 4),
              valueBoxOutput("confidence",width = 2),
              valueBoxOutput("fluency",width = 2),
              valueBoxOutput("energetic",width = 2),
              valueBoxOutput("clarity",width = 2)),
            ...
            fluidRow(
              ...
              box(
                title = "Random Forest",
                tabsetPanel(
                  tabPanel("Results",tableOutput("randomforest_results")),
                  tabPanel("Radar Chart",plotOutput("rf_plot"))
                ),
                height = 500
              )#end of box
...
server <- function(input, output) {
  ...
  extraction2<-reactive(
    {
      ...
      withProgress(message = 'Analysis Progress',value=0,{
        ...
        incProgress(0.3,detail = "Sound file creation done....")
        #extract
        #pitch
        ...
        #diffIntMaxMode Here
        incProgress(0.2,detail = "Extracting details...")
        ...
        incProgress(0.2,detail = "Analysing voice...")
        ...
        incProgress(0.2,detail = "Predicting your results...")
        #Jitter
        ...
        incProgress(0.1,detail="file extraction done....")
        Sys.sleep(0.1)
      })
      temp_extract=1
      return(temp_extract)
  ...
  output$rf_plot<-renderPlot({
    FullPath = function(FileName){ return( paste( "D:/BE_Project/BE/finalappmit/", FileName, sep=""))}
    svm_result<-data.frame(read.csv(file=FullPath("Rf_result.csv"),header = T,stringsAsFactors = F))
    ...
    return(radarchart( attri  , axistype=1 ,
                       ...
                       #custom the grid
                       cglcol="red", cglty=1, axislabcol="orange", caxislabels=seq(0,10), cglwd=0.8,
  ...
  output$oi<-renderValueBox({
      ...
      Confidence<-(rf_result$Confidence*3)/10
      Fluency<-(rf_result$Fluency*3)/10
      Energetic<-(rf_result$Energetic*3)/10
      Clarity<-(rf_result$Clarity*3)/10
      models<-data.frame(Confidence,Fluency,Energetic,Clarity)
      svm_model_after_tune<-readRDS(FullPath("rf_oi.rds"))
      oi_pred <- predict(svm_model_after_tune,models)
      oi_pred <- as.numeric(oi_pred)
      oi_pred<-floor(oi_pred)
      valueBox(
        paste0(oi_pred, "/10"), "Oration Index",
        color = "lime"
      )
    })
  ...
  output$resume_developer<-renderPlot(
    {
      ...
      plot_dev<-doughnut(c(rating_dev,10-rating_dev) ,labels=c("Development","."),init.angle = 0, col=c("steelblue1","snow2") )
      return(plot_dev)
    },
    height=350
  )
  ...
  output$resume_admin<-renderPlot(
    {
      ...
      plot_admin<-doughnut( c(rating_admin,10-rating_admin) , labels=c("Administration","."),init.angle = 0 ,col=c("darkolivegreen2","snow2") )
  ...
  output$resume_analyst<-renderPlot(
    {
      ...
      new.data <- rbind(existing.data, data_to_be_added)
      write.xlsx(new.data, file, sheetName = SHEET.NAME, row.names = F, append = F)
      plot_analyst<-doughnut( c(rating_analyst,10-rating_analyst) ,labels=c("Analysis","."),init.angle = 0, col=c("coral4","snow2") )
      return(plot_analyst)
    }
  )
